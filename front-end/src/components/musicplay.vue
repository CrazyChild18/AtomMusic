<template>
    <div class="musicPlay">
        ...
        <!-- 播放进度条 -->
        <span style="position: absolute;right: 72%;bottom: 14%;">{{musicDuration || timeFormat}}</span>
        <div id="playSlider">
            <el-slider v-model="musicDuration" :max="musicAllDuration" :show-tooltip="false" @change="changeMusicDuration" @mousedown="isChange = true" @mouseup="isChange = false"></el-slider>
        </div>
        <span style="position: absolute;left: 72%;bottom: 14%;">{{musicAllDuration || timeFormat}}</span>
    </div>
</template>
