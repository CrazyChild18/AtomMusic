...
class MakeRecommendation(object):
    ...
    def recommend(self, genre_name):
        ...
        # print(tracks_information[:4]['track_url'])
        # print(tracks_information[:4]['track_name'])

        # 给出 TOP 推荐
        top_4_recs = list(recommendations.head(5)['index'])[1:]
        d_list = []
        for r in top_4_recs:
        ...
        for item in d_list:
            item = item[0]
            song_strip = re.findall(reg, item['track_name'])[0]
            # dic = {'song': song_strip, 'artist': item['release_artist'], 'url': item['track_url']}
            dic = {'song': song_strip, 'artist': item['release_artist']}
            ret_list.append(dic)
